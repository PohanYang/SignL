...
#def head_init(show_frame, detect_postion, head_postion, head_count, head_lock, head_succ):
#    head_postion = np.zeros(4)
#    if np.count_nonzero(detect_postion)!=4 and head_lock==False:
#        show_frame = printing_word(show_frame,"Plz stand in front of webcam", 40, 10, 5)
#    if np.count_nonzero(detect_postion)==4 and head_lock==False:
#        head_count += 1
#        show_frame = printing_word(show_frame,"Prepare to detect head...", 40, 10, 5)
#    if head_count >= 100 and  head_succ==False:
#        head_lock = True
#        head_count += 1
#        show_frame = printing_word(show_frame,"Detect head successful!", 40, 10, 5)
#    if head_count == 130:
#        head_succ = True
#        head_postion=[detect_postion[0][0]+(detect_postion[0][2]/2), detect_postion[0][1]+(detect_postion[0][3]/2)]
#    return show_frame, head_postion, head_count, head_lock, head_succ
...
while(1):
	...
	#	    t_frame[i,j,:]=0
        ...
        if righthand[0]!=0:
            ...
            rightimg = Image.fromarray(rightframe, 'RGB')
            rightimg = rightimg.resize((60,50), Image.BILINEAR)
            rightflat_arr = np.array(rightimg)
            rightflat_arr = np.reshape(rightflat_arr,[-1,50,60,3])
            #rightflat_arr = rightarr.ravel()
            #rightflat_arr = rightflat_arr.reshape((1,9000))
            ...
            #show_frame = printing_word(show_frame, int(ans[0]), 3, frame.shape[1]-85)
        #if head_lock==True and head_succ==True:
            print right_ans[0]
        ...
        if False:
            ...
            #leftflat_arr = leftarr.ravel()
            #leftflat_arr = leftflat_arr.reshape((1,9000))
            # Display the resulting frame
        ...
        #show_frame = printing_word(show_frame, int(ans[0]), 3, frame.shape[1]-85)
        #if head_lock==True and head_succ==True:
            #show_frame = printing_word(show_frame, "Your left Sign is: "+str(int(left_ans[0])), 60, 3, 470)
